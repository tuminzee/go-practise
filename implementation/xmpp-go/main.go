...
func main() {
	...
	recipientJID := jid.MustParse("user@domain")
	...
	for {
		...
		err := s.Encode(context.TODO(), MessageBody{
			...
			Body: `Hello world! time: ` + time.Now().Format(time.RFC3339),
		})
		if err != nil {
			log.Printf("error sending message to %s: %v", recipientJID, err)
		}
	}
}
